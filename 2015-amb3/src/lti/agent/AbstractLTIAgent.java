...
public abstract class AbstractLTIAgent<E extends StandardEntity> extends
	...
	protected Map<EntityID, Set<EntityID>> knownAgents;
	...
	protected void postConnect() {
		...
		knownAgents = new HashMap<EntityID, Set<EntityID>>();
	 ...
	 * @param changed
	 */
	private void readMsgAgentPosition(Parameter param) {
		if (!(param instanceof AgentPosition)){
			this.toWrite +="\n VOLTOU \n";
			return;
		}
		//this.toWrite+="\nTESTE\n";
		AgentPosition agentPosition =  (AgentPosition) param;
		StandardEntityURN urn = agentPosition.getURN();
		
		//this.toWrite+="\n AGENTE "+agentPosition.toString()+"\n";
		
		EntityID entity = new EntityID(agentPosition.getAgent());
		EntityID position = new EntityID(agentPosition.getPosition());
		
		//this.toWrite+="AGENTE"+urn.toString()+"EU"+model.getEntity(this.getID()).getStandardURN().toString()+"\n";
		
		if (urn.equals(model.getEntity(this.getID()).getStandardURN())){
			if(!knownAgents.containsKey(agentPosition.getAgent())){
				knownAgents.put(entity, new HashSet<EntityID>());
			}
			knownAgents.get(entity).clear();
			knownAgents.get(entity).add(position);
			
			StandardEntity agent = model.getEntity(entity);
			Human human;
		
			if (agent != null && agent instanceof Human) {
				switch (agent.getStandardURN()){
				case FIRE_BRIGADE:
					human = (FireBrigade) agent;
					break;
				case POLICE_FORCE:
					human = (PoliceForce) agent;
					break;
				case AMBULANCE_TEAM:
					human = (AmbulanceTeam) agent;
					break;
				default:
					human = (Civilian) agent;
					break;
				}
			}else{
				switch (urn) {
				case FIRE_BRIGADE:
					human = new FireBrigade(entity);
					break;
				case POLICE_FORCE:
					human = new PoliceForce(entity);
					break;
				case AMBULANCE_TEAM:
					human = new AmbulanceTeam(entity);
					break;
				default:
					human = (Civilian) agent;
					break;
		...
		human.setPosition(new EntityID(agentPosition.getPosition()));
		if (model.getEntity(human.getPosition()) instanceof Area) {
			Area a = (Area)model.getEntity(human.getPosition());
			human.setX(a.getX());
			human.setY(a.getY());
		...
		/*Human h = (Human)model.getEntity(position);
		toWrite+="\n"+h.getX()+","+h.getY()+"\n";*/
